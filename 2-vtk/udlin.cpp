...
public:
    ...
    CalcMesh(const std::vector<double>& nodesCoords, const std::vector<std::size_t>& tetrsPoints) {
        ...
        for(unsigned int i = 0; i < nodesCoords.size() / 3; i++) {
            ...
            // Cкорости на ...
            double velocityX = pointX;
            double velocityY = 0.0;
            double velocityZ = 0.0;
            // Модельная скалярная величина распределена как-то вот так
            double smth = -sin(0.25*pointX);
            nodes[i] = CalcNode(pointX, pointY, pointZ, smth, velocityX, velocityY, velocityZ);
        }
    ...
    void snapshot(unsigned int snap_number) {
        ...
        string fileName = "udlin-step-" + std::to_string(snap_number) + ".vtu";
...
int main()
{
    ...
    for(unsigned int step = 1; step < 110; step++) {
